    ...
    return (
        <AppPageShell name="ExerciseDetailPage">
            <main className="flex flex-col gap-y-4 px-6 py-6">
                <span className="text-3xl font-extrabold tracking-tight">{exercise.name}</span>

                <span>Type: {displayExerciseType(exercise.type)}</span>

                {exercise.target_muscle_group && <div>Target Muscle Group: {exercise.target_muscle_group.name}</div>}

                {exercise.equipment && <div>Equipment: {exercise.equipment.name}</div>}

                {exercise.force && <div>Force: {displayExerciseForce(exercise.force)}</div>}
                ...
                {exercise.instructions.length && <span className="text-xl font-medium">Instructions</span>}
