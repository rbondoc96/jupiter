...
const buttonStyles = cva(['relative', 'border-2', 'self-center', 'text-center'], {
    variants: {
        size: {
            md: ['rounded-lg', 'px-5 py-3', 'text-sm md:text-base'],
        },
        variant: {
            primary: ['bg-primary hover:bg-transparent', 'border-primary', 'text-white hover:text-primary'],
            primaryGhost: ['bg-transparent', 'border-primary', 'text-primary'],
            unstyled: ['border-0', 'rounded-none', 'p-0'],
        },
    },
    defaultVariants: {
        size: 'md',
    ...
    (
        ...
        return (
            <Component
                type={type}
                ref={ref}
                className={composeClassName(buttonStyles({ size, variant }), classNames?.root)}
                onClick={onClick}
                onKeyDown={onKeyDown}
                {...(asChild ? {} : props)}
            >
                {isLoading && (
                    ...
                    >
                        <FontAwesomeIcon
                            className={composeClassName('animate-spin text-white', classNames?.actionIndicator)}
