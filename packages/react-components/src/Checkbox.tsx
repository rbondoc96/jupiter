...
export const Checkbox: FunctionComponent<CheckboxProps> = ({
    ...
    return (
        <div className={composeClassName('flex items-center gap-x-2', classNames?.root)}>
            <RadixRoot
            ...
            >
                <RadixIndicator className="flex">
                    {checked === undefined || checked === 'indeterminate' ? (
                        <FontAwesomeIcon className="h-3 w-3" icon={faSlash} />
                    ) : (
            ...
            <label className={composeClassName('text-sm', name && 'cursor-pointer', classNames?.label)} htmlFor={name}>
                {label}
            </label>
        </div>
    );
